...
result=$(ssh -o 'ProxyCommand aws ssm start-session --target ${EC2_INSTANCE_ID} --document-name AWS-StartSSHSession --parameters 'portNumber=22'' ${USERNAME}@${EC2_PUBLIC_IP} "${COMMAND}")
